...
class SDKServer {
  ...
  async verifySession(
    ...
    try {
      ...
      // name pode ser string vazia (usuários sem nome definido) — só openId e appId são obrigatórios
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    if (!user) {
      // Email/password users have openId starting with "email_" — don't try OAuth sync
      const isEmailUser = sessionUserId.startsWith("email_");
      if (!isEmailUser) {
        try {
